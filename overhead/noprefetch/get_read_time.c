...
int main (int argc, char *argv[])
{
    ...
    int i, j;
    double start_time, elapsed_time, all_time_max, all_time_avg, all_time_min;
    MPI_Status status;
    MPI_File fh;
...
//    printf("%d: %lf\n",my_rank, elapsed_time);
    ...
    double time_per_read = all_time_max / repeat_time;
    // output average read time for prefetch to use
    if(my_rank == 0) {
        MPI_File_open(MPI_COMM_SELF, "time_per_read", MPI_MODE_CREATE | MPI_MODE_RDWR, MPI_INFO_NULL, &fh);
        MPI_File_write( fh, &time_per_read, 1, MPI_DOUBLE, &status );
        MPI_File_close(&fh);
    }
    
    if(my_rank == 0) {
        time_per_read = 0.0;
        MPI_File_open(MPI_COMM_SELF, "time_per_read", MPI_MODE_RDWR, MPI_INFO_NULL, &fh);
        MPI_File_read( fh, &time_per_read, 1, MPI_DOUBLE, &status );
        printf("time:%lf\n",time_per_read);
        MPI_File_close(&fh);
    }
