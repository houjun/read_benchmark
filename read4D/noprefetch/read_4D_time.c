...
//            printf("CUBE ");
    ...
    if(my_rank == 0)
         printf("x:%d y:%d z:%d b:%d time_step:%d\n", x, y, z, b, time_step);
    ...
    if(my_rank == 0)
        printf("Total time: %lf Min time: %lf Avg time: %lf Total data: %dM Agg Bandwidth: %lf Repeat Time:%d\n"
                , all_time, all_time_min, all_time_avg, (int)data_in_mb, data_in_mb/all_time, repeat_time);

    if(my_rank == 0) {
        double time_per_read = all_time / repeat_time;
        FILE *fp = fopen(argv[8], "w");
        fprintf(fp, "%lf",time_per_read);
        fclose(fp);
    }
